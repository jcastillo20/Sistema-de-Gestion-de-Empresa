...
### 6. Módulo de Paquetes (Estructura Enterprise)
Se ha implementado una división de seguridad basada en los requerimientos de auditoría y RBAC:

*   **Módulo Catálogo (`MOD_PAQUETES_CATALOGO`)**: Reservado para administradores. Permite definir la "Matriz de Productos" (Precio, Sesiones, Reglas).
*   **Módulo Control (`MOD_PAQUETES_CONTROL`)**: Centro de operaciones de ventas. 
    *   **Buscador Global**: Filtros cruzados por Paciente, Sede y Frecuencia.
    *   **Buscador Predictivo**: Selección de pacientes mediante ComboBox con filtrado en tiempo real.
*   **Historial Dinámico**: En la ficha del paciente se aplica la regla de "Top 4 Cards" para acceso rápido, relegando el resto al historial para optimizar el espacio visual (UX Fit-to-Screen).
*   **Garantía de Inmutabilidad**: Cada venta genera un "Snapshot" de las condiciones comerciales, protegiendo la integridad financiera ante cambios en el catálogo.
